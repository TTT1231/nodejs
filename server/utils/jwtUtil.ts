...
import { JwtPayload } from "server/types/jwtTypes";
...
function getAccessSecretBase64() {
    if (accessSecretBase64===undefined) {
        accessSecretBase64 = textEncoder.encode(getRequiredEnv('NODE_JWT_ACCESS_SECRET'));
    }
    return accessSecretBase64;
}
function getRefreshSecretBase64() {
    if (refreshSecretBase64 ===undefined) {
        refreshSecretBase64 = textEncoder.encode(getRequiredEnv('NODE_JWT_REFRESH_SECRET'));
    }
